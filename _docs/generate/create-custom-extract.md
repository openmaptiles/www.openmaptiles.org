...
# The legacy site's only FAQ entry was about extract/planet refresh cadence, which
# belongs with this page. See the "Data updates" section below.
redirect_from: /faq/
---

If you need an extract that is not included on the [downloads page](https://www.maptiler.com/on-prem-datasets/planet/), you need to download the planet file and create your own MBTiles extract. If you need a smaller area, then it is available as OSM.PBF extract, e.g., on [Geofabrik](https://geofabrik.de/). Extracting could be done before importing into a database.
...
The extracts and the planet download are refreshed weekly from the newest
OpenStreetMap planet dump, so a re-download picks up recent OSM edits without any
change to the commands above.
